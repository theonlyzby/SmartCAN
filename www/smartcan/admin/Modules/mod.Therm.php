...
function Therm() {
  ...
  while(!feof($reading)) {
	...
	if (strpos($line,"              <p id='nestTitle' class='nestTitle'>")!==false)  { $nestName = substr($line, 50, strrpos($line,"<")-50); }
  } // END WHILE
  fclose($reading);
  ...
  if (($nestName!=$Form_nestName) &&($Form_nestName!="")) {
	...
	file_put_contents($myFile,str_replace("<p id='nestTitle' class='nestTitle'>".$nestName."</p>","<p id='nestTitle' class='nestTitle'>".$Form_nestName."</p>",file_get_contents($myFile)));
  ...
  if ($action=="ChangeOWGPIO") {
	...
	if ($OWGPIO!=$GPIOpin) {
	  ...
	  $reading   = fopen($myFile,'r');
	  $writing   = fopen("/var/www/smartcan/dists/config.tmp","w");
	  while(!feof($reading)) {
	    $line = fgets($reading,4096);
	  ...
	  fclose($reading); 
	  fclose($writing);
	  shell_exec("sudo cp -f /var/www/smartcan/dists/config.tmp /boot/config.txt");
	  shell_exec("sudo rm -f /var/www/smartcan/dists/config.tmp");
 ...
 foreach($lines as $oneline) {
  ...
  echo("<div id=\"DivOneWire\" " . $sel_OW . ">".$msg["THERM"]["OWSensor"][$Lang].": <select name='ExtOneWire' id='ExtOneWire'>" . CRLF);
