		...
		// Mode 2 Circulateurs (1er Contact = Circulateur Chauffage), 2eme contact = Circulateir Boiler) //
		 /////////////////////////////////////////////////////////////////////////////////////////////////  
		  ...
		  if (((( $voulu2 >= $moyenne_actuelle && $moyenne_actuelle != '0' ) && ($absence==0 && $periode_chauffe>=1)) || ($tempmini>$moyenne_actuelle && $moyenne_actuelle!='0')) 
			...
			if ( $chaudiere == '0' ) {
			  if (DEBUG_AJAX) { mysqli_query($DB,"INSERT INTO `logs` (`id_gradateur`, `id_sortie`, `date`, `valeur`) VALUES ('" . CARTE_CHAUFFAGE . "','" . SORTIE_CHAUFFAGE . "', CURRENT_TIMESTAMP, '32');"); }
			  mysqli_query($DB,"UPDATE `" . TABLE_CHAUFFAGE_CLEF_TEMP . "` SET `valeur` = '1' WHERE `clef` = 'chaudiere';");
		  ...
		  if (( ($moyenne_actuelle >= $voulu3 && $tempmini<$moyenne_actuelle+1) || $moyenne_actuelle == '0' ) || ($absence==1 || $periode_chauffe==0) || ($periode_boiler>=1)) {
			...
			if ($absence==0 && $periode_boiler>=1) { // && $warm_water==0) {
			  ...
			  if ($etat_boiler == '0') {
				if (DEBUG_AJAX) { mysqli_query($DB,"INSERT INTO `logs` (`id_gradateur`, `id_sortie`, `date`, `valeur`) VALUES ('" . CARTE_BOILER . "','" . SORTIE_BOILER . "', CURRENT_TIMESTAMP, '32');"); }
				mysqli_query($DB,"UPDATE `" . TABLE_CHAUFFAGE_CLEF_TEMP . "` SET `valeur` = '1' WHERE `clef` = 'boiler';");
				if (DEBUG_AJAX) { mysqli_query($DB,"INSERT INTO `" . TABLE_MEASURE . "` (`measure_type`, `start_time`, `start_value`) VALUES ('BOILER',CURRENT_TIMESTAMP, '0');"); }
			  } // ENDIF
			} // ENDIF
			...
			if ($absence==1 || $periode_boiler==0) { // || $warm_water==1) {
			  ...
			  if ($etat_boiler == '1') {
				if (DEBUG_AJAX) { mysqli_query($DB,"INSERT INTO `logs` (`id_gradateur`, `id_sortie`, `date`, `valeur`) VALUES ('" . CARTE_BOILER . "','" . SORTIE_BOILER . "', CURRENT_TIMESTAMP, '00');"); }
