/*  DOMOCANv3 linux Bridge by Henry2.0
...
int main(int argc, char **argv) {
  ...
  int i, s, nbytes, ret, domogest_ack = 0;
  ...
  char temp_msg[64], fcs[2], msg_out[40], tmp_string[40], frm_content[80], buf[10], domogest_msg[64];
...
// Fill out hints for getaddrinfo().
  ...
  while (1) {
	...
	// Determine Length (dlc+4)
		...
		// CMD 0x18 on GRAD16 or IN16 ? ... PUSH and update DB
		if ((!strcmp(SID,"5018")) || (!strcmp(SID,"6018"))) { 
		  //sprintf(cmd, "nohup php5 /var/www/smartcan/bin/recv.php %s > nohup.out 2>&1 &", DomoCANframe); // " > /dev/null 2>&1"
		  sprintf(cmd, "php /var/www/smartcan/bin/rx-DOMOCAN3.php %s", DomoCANframe); // " > /dev/null 2>&1"
          system(cmd);
		} // ENDIF

		// ACK to confirm UDP Reception toward Domogest Required?
		if (domogest_ack=='1') {
		  domogest_ack = 0; printf("\n DomoGest ACK to send! \n");
		  
		  raw_len = convert_raw(domogest_msg, msg_out);
          printf("\n=>TX UDP CAN Send OK=Frame 0x50): %s",domogest_msg);
		  // Prepare packet.
...
//      if (read(sa, udpframe, MAXDG)!=0) {
      ...
      if (!strcmp(FCS, calc_FCS)) {
        ...
        if (!strcmp(UDP_CMD, "60")) {
          ...
          domogest_msg[0]='\0';
		  sprintf(domogest_msg, "%s%s%s", "50", PCID, "01",  FCS);		  
		  // Dummy bytes
		  for (i=6; i<=29;i++) {
		    domogest_msg[i]='0';
		  } // END FOR
         ...
         // Calculating checksum (FCS)
		  domocan_checksum(domogest_msg, fcs);
          strcat(domogest_msg, fcs); domogest_msg[32]='\0';
