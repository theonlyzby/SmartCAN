...
class zWave_class {
  ...
  function HTMLconfig() {
	  ...
	  echo('<td width="50%"><select id="ZoneID" name="ZoneIDzWave" onchange="NewZonezWave()" class="mapped" select-group="ZoneID"><option value="">Select</option>');
	  ...
	  echo('<option value="0" onselect="javascript:NewZonezWave();">' . $msg["TEMPS"]["NewZone"][$Lang] . '</option></select></td></tr>' . CRLF);
	  ...
	  echo('<td width="50%"><input id="Zone_NamezWave" name="Zone_NamezWave" type="text"/></td></tr>' . CRLF);
	  ...
	  echo('<td width="50%"><select id="ZoneNberzWave" name="ZoneNberzWave"><option value="">Select</option>' . CRLF);
	  $sql = "SELECT * FROM `ha_thermostat_zones`;";
	  $query = mysqli_query($DB,$sql);
	  while ($row = mysqli_fetch_array($query, MYSQLI_BOTH)) {
		if ($row["Name"]=="") {
  ...
  function javaChange() {
      ...
      echo('}' . CRLF);
      echo('function NewZonezWave() {' . CRLF);
	  echo('  var value = document.getElementById("ZoneIDzWave").value;');
      echo('  if (value==0) { document.getElementById("NewzWave1").style.display = "table-row";document.getElementById("NewzWave2").style.display = "table-row";  document.getElementById("Zone_NamezWave").focus();}' . CRLF);
  ...
  function HTMLcheck() {
	...
	//echo('alert("id sonde="+t.substring(0,5));' . CRLF);
	echo(' if (t.substring(0,5)=="zWave") {  ' . CRLF);
	//echo('alert("zWave Check");' . CRLF);
    echo('  var e = document.getElementById("ZoneIDzWave");' . CRLF);
    ...
    echo('  var e = document.getElementById("ZoneNberzWave");' . CRLF);
    echo('  var ZoneNber = e.options[e.selectedIndex].value;' . CRLF);
	echo('  document.getElementById("ZoneNber").value = ZoneNber;' . CRLF);
	
	echo('  if ((document.getElementById("NewzWave1").style.display=="table-row") && ((document.getElementById("Zone_NamezWave").value=="") || (ZoneNber==""))) { alert("' . $msg["TEMPS"]["ZoneSelectError"][$Lang] .'");return; }' . CRLF);
    echo('  var e = document.getElementById("id_sonde");' . CRLF);
	echo('  var t = e.options[e.selectedIndex].text;' . CRLF);
