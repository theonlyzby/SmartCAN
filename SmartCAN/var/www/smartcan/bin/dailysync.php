...
while ($row = mysqli_fetch_array($query, MYSQLI_BOTH)) {
  ...
  if (($daily_backup=="Y") || (($weekly_backup=="Y") && (date("N")=="5")) || (($monthly_backup=="Y") && (date("d")=="01"))) {
    ...
    exec("mysqldump --opt --user=" . mysqli_LOGIN . " --password='".mysqli_real_escape_string($DB,mysqli_PWD)."' " . mysqli_DB . " > " . $BackupDest . "domotique.sql");
	exec("mysqldump --user=" . mysqli_LOGIN . " --password='".mysqli_real_escape_string($DB,mysqli_PWD)."' mysql user > " . $BackupDest . "mysql.sql");
	exec("pdbedit -e smbpasswd:" . $BackupDest . "samba-users.smbback");
	$fileName = date("Ymd-His-").'FULLsmartCAN-BACKUP.tar.gz';
	exec('tar --exclude="' . $base_URI   . '/sys-files/node-red/node_modules" --exclude="' . $base_URI   . '/sys-files/zigbee2mqtt/data/log" -czvf '. $BackupDest.$fileName . ' ' .
	...
	exec('rm ' . $BackupDest . 'mysql.sql');
	exec('rm ' . $BackupDest . 'samba-users.smbback');
