...
class Zigbee_class {
  ...
  function get_Temp($sensor) {
	...
	//echo("Node=".$node.CRLF);
	$zone = $row["moyenne"]; if ($zone=="1") { $zone_select = "`active`='Y'"; } else { $zone_select = str_pad("",(intval($zone)-1),"_")."1".str_pad("",(7-intval($zone)),"_"); }
	$id   = $row["id"];
    ...
    $Away = $row3['valeur'];
	//echo("Timeslots?".$row2["County"].",Away?".$Away . CRLF);
    ...
    curl_close($ch);
    # Print response.
    //echo "Zigbee for ".$node.", temp=".$temp." => $result \n";
