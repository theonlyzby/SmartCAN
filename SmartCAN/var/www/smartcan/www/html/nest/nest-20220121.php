...
if (currentMode!="HEAT NOW") {
  ...
  $("#hour2" ).slideDown();
  $("#consignee" ).slideDown();
  $( "#nestMode" ).html("33TEMP (" + temperatureConfort + "&deg;)");
...
// Une petite tache de fond lancée toutes les 5 secondes pour rafraichir les données depuis le serveur
...
function sensorFresh() {
	if ( readyToSensor == "OK" ) {
	  if (window.parent.divabsence.innerHTML!="1") {
		...
		$( "#nestMode" ).html("TEMP (" + temperatureNest + "&deg;)");
...
// Fonction mettant à jour les barres en passant la valeur et le ratio
